...
def test_exact_reference_match_stays_in_high_sanity_band(tmp_path: Path) -> None:
    ...
    result = OfflineReferenceAnalysisPipeline().analyze(
        AnalyzeRequest(
    ...
    assert _overall_score(result.report) >= expected["overall_score_min"]
    assert _score_by_name(result.report, MetricName.PITCH_FIDELITY) >= expected["pitch_fidelity_min"]
