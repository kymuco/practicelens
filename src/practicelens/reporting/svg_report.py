...
def report_to_svg(report: AnalysisReport) -> str:
    ...
    parts.append('<text x="24" y="36" font-size="22" font-family="Arial">PracticeLens Summary</text>')
    parts.append(
        f'<text x="24" y="58" font-size="14" font-family="Arial">Overall score: {overall_score:.1f}/100</text>'
    )
    ...
    for index, score in enumerate(report.scores):
        ...
        parts.append(
            f'<text x="24" y="{y + 17}" font-size="13" font-family="Arial">{escape(score.name.value)}</text>'
        )
        parts.append(
            f'<rect x="{bar_left}" y="{y}" width="{bar_width}" height="{bar_height}" fill="#e5e7eb" rx="4" />'
        )
        parts.append(
            f'<rect x="{bar_left}" y="{y}" width="{filled_width:.1f}" height="{bar_height}" fill="#4f46e5" rx="4" />'
        )
        parts.append(
            f'<text x="{bar_left + bar_width + 12}" y="{y + 17}" font-size="13" font-family="Arial">{score.score:.1f}</text>'
        )

    section_base_y = 270
    parts.append(
        f'<text x="24" y="{section_base_y - 16}" font-size="16" font-family="Arial">Section averages</text>'
    )
    if report.sections:
        chart_left = 40
        ...
        chart_height = 100
        parts.append(
            f'<rect x="{chart_left}" y="{chart_top}" width="{chart_width}" height="{chart_height}" fill="#f9fafb" stroke="#d1d5db" />'
        )
        section_points = []
        ...
        for x, y, section_index, section_avg in section_points:
            ...
            parts.append(
                f'<text x="{x:.1f}" y="{chart_top + chart_height + 18}" text-anchor="middle" font-size="12" font-family="Arial">S{section_index}</text>'
            )
            parts.append(
                f'<text x="{x:.1f}" y="{y - 8:.1f}" text-anchor="middle" font-size="11" font-family="Arial">{section_avg:.0f}</text>'
            )
